...
\midrule
Reestruturação da Etapa 1 e da Etapa 2, mantendo artefatos separados por execução & Corpus \emph{Portuguese Tweets for Sentiment Analysis} no Kaggle \citep{augustopKaggleTweets} \\
...
O segundo processo é o tratamento textual. O modo bruto preserva o corpus original. Para diagnóstico, foram implementadas duas versões tratadas: uma remove emoticons e URLs, que são os símbolos mais diretamente associados à supervisão distante, e outra também remove menções, hashtags e marcadores de fontes neutras recorrentes. A função desses tratamentos não é ``corrigir'' definitivamente o corpus, mas medir quanto do desempenho depende de atalhos superficiais.
...
O quarto processo é a avaliação. O projeto salva métricas agregadas, F1 por classe, matriz de confusão, predições e casos de erro em pastas separadas por execução. Essa organização evita que execuções da Etapa 1, da Etapa 2, de diagnósticos e de transformers sobrescrevam artefatos uns dos outros.
...
Os baselines clássicos usam vetorização TF-IDF com unigramas e bigramas de palavras. A Regressão Logística funciona como baseline simples e interpretável; o Linear SVM funciona como baseline clássico forte para texto esparso, frequentemente competitivo em tarefas de classificação textual.
...
O conjunto de treino tem 100.000 tweets e o conjunto de teste tem 4.999 tweets. A Tabela~\ref{tab:dataset} resume os tamanhos e a distribuição das classes.
...
\midrule
2442 & Simbólico falha, TF-IDF acerta & \texttt{poxa ((} & negativo & simbólico: neutro; Regressão Logística: negativo; SVM linear: negativo & O léxico não encontra uma pista polarizada forte depois da remoção do emoticon, enquanto os baselines estatísticos aprendem o padrão recorrente de lamento. \\
1300 & Simbólico falha, TF-IDF acerta & \texttt{É hoje!} & positivo & simbólico: neutro; Regressão Logística: positivo; SVM linear: positivo & O exemplo ilustra uma frase curta sem palavra positiva explícita no OpLexicon, mas frequente em contextos positivos do corpus. \\
2990 & TF-IDF falha em ambiguidade & \texttt{to feliz e com medo ao mesmo tempo} & negativo & Regressão Logística: positivo; Albertina: negativo; baseline de pistas: negativo & A frase mistura \texttt{feliz} e \texttt{medo}. Sem o emoticon negativo original, a Regressão Logística pesa mais a pista positiva e erra. \\
295 & TF-IDF falha em ironia/contradição & \texttt{Tô de boa sim po Encolhida Chorando Nada de demais} & positivo & Regressão Logística: negativo; Albertina: negativo; baseline de pistas: positivo & A frase parece irônica e contém \texttt{Chorando}, mas o rótulo positivo vinha do emoticon original. O caso mostra que a remoção de pistas também revela inconsistências do próprio corpus. \\
3798 & Modelo bruto parece melhor por vazamento & \texttt{Overdose deixou Demi Lovato à beira da morte} & neutro & TF-IDF bruto: neutro; baseline de pistas: neutro; TF-IDF tratado: negativo & No texto original havia uma URL. Como URLs quase sempre marcam a classe neutra no corpus, o acerto bruto não depende de compreender a manchete. \\
\bottomrule
\end{tabularx}%
}
