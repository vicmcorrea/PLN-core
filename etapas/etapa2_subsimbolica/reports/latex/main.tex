...
Os modelos neurais avaliados são DistilBERT multilíngue, XLM-R base e Albertina-100M pt-BR. Um transformer é um modelo neural pré-treinado em grande volume de texto, capaz de produzir representações contextuais para palavras e subpalavras. Ajuste supervisionado, neste trabalho, significa continuar o treinamento desse modelo usando os tweets rotulados do corpus e substituir a saída original por uma camada de classificação em três classes. O DistilBERT foi usado porque é uma versão destilada e mais leve da família BERT \citep{sanh2019distilbert}. XLM-R foi usado porque é um modelo multilíngue forte, adequado quando se quer comparar português com uma arquitetura treinada em muitos idiomas \citep{conneau2020xlmr}. Albertina foi usada porque é uma alternativa voltada ao português e, portanto, testa se pré-treinamento mais próximo da língua-alvo ajuda no cenário limpo \citep{rodrigues2023albertina}.
...
\addlinespace[0.3em]
OpLexicon v3.0 & Léxico de polaridade para português & Fonte das polaridades usadas pelo sistema simbólico com regras \\
\addlinespace[0.3em]
...
\addlinespace[0.3em]
Hugging Face Transformers & Biblioteca para tokenização e ajuste de transformers & Execução de DistilBERT, XLM-R e Albertina como modelos neurais de benchmark \\
\addlinespace[0.3em]
