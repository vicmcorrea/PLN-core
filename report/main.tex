...
\begin{itemize}
  \item expressões regulares para normalização do texto
...
O sistema utiliza o OpLexicon v3.0 como recurso externo de polaridade. Os dados são carregados automaticamente e entradas duplicadas são agregadas por média de polaridade, produzindo o escore lexical usado nas etapas seguintes do pipeline.
...
Busca no léxico & \texttt{bom} $(+1{,}0)$, \texttt{mas} $(-1{,}0)$ e \texttt{confuso} $(-1{,}0)$ são encontrados no OpLexicon \\
Regras aplicadas & \texttt{bom}: intensificação, pré-contraste e exclamação; \texttt{mas}: exclamação; \texttt{confuso}: pós-contraste e exclamação \\
Contribuições ajustadas & \texttt{bom}: $+1{,}176$; \texttt{mas}: $-1{,}050$; \texttt{confuso}: $-1{,}365$ \\
Agregação e classificação & soma final $-1{,}239$, classificada como \textbf{negativo} \\
\bottomrule
\end{tabularx}
\end{center}

Esse exemplo ilustra como uma palavra positiva inicial pode ser superada por termos negativos posteriores ao marcador de contraste, produzindo uma decisão final negativa.
